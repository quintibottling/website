...
export function resolveLink(model, locale, link, productSlug = null) {
  if (model == "product") {
    return `/${translate("products_url", locale)}/${link}`;
  }
  if (model == "contact_page") {
    return `/${translate("contacts_url", locale)}/${link}`;
  }
  ...
  if (model == "machine") {
    return `/${translate("products_url", locale)}/${productSlug}/${link}`;
  } else return `/${link}`;
}
