...
query homepage($locale: SiteLocale!) {
  ...
  news: allNews(first: "2", orderBy: _createdAt_DESC, locale: $locale) {
    category {
      slug
      title
    }
...
query news($slug: String!, $locale: SiteLocale!){
  allNews(filter: {slug: {neq: $slug}}, locale: $locale, first: "2", orderBy: _createdAt_DESC) {
    category {
      slug
      title
    }
...
query product($slug: String!, $locale: SiteLocale!){
  ...
  allTecnology: allTecnologyDetails(locale: $locale) {
    id
    title
    subtitle
    model: _modelApiKey
