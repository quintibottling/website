...
const textImageTwoColumn = `
  ...
  banner:image {
    responsiveImage(sizes: "(min-width: 768px) 50vw, 100vw, 1600px", imgixParams: {auto: [format, compress], fit: max}){
      ...imgFrag
    }
  }
`;
...
query homepage($locale: SiteLocale!) {
  ...
  news: allNews(first: "2", locale: $locale, filter: {slug: {neq: null}}, orderBy: _createdAt_DESC) {
    category {
      slug
      title
    }
...
query allCompanyPages ($locale: SiteLocale!){
  companyPages: allCompanyPages (locale: $locale, filter: {slug: {neq: null}}) {
    slug
    model: _modelApiKey
  }
}
...
query allEditorialPages ($locale: SiteLocale!){
  editorialPages: allEditorialPages(locale: $locale, filter: {slug: {neq: null}}) {
    slug
    model: _modelApiKey
  }
}
...
query allTecnologies($locale: SiteLocale!){
  tecnologies: allTecnologies(filter: {slug: {neq: null}}, locale: $locale) {
    slug
    model: _modelApiKey
  }
}
...
query allNews($locale: SiteLocale!, $offset: IntType, $first: IntType = "100"){
  allNews(locale: $locale, first: $first, skip : $offset, filter: {slug: {neq: null}}) {
    slug
    id
    model: _modelApiKey
  }
...
query allNews($locale: SiteLocale!, $offset: IntType, $first: IntType = "100") {
  allNews(locale: $locale, first: $first, skip : $offset, orderBy: _createdAt_DESC, filter: {slug: {neq: null}}) {
    id
    model: _modelApiKey
    title
    slug
...
query indexNews ($locale: SiteLocale!, $skip: IntType, $first: IntType = "100"){
  indexNews (locale: $locale){
    title
    slug
    labelMenu
  ...
  news: allNews(locale: $locale, orderBy: _createdAt_DESC, first: $first, skip: $skip, filter: {slug: {neq: null}}) {
    category {
      slug
      title
    }
...
query allProducts($locale: SiteLocale!){
  products: allProducts(locale: $locale, filter: {slug: {neq: null}}) {
    slug
    id
    title
    model: _modelApiKey
...
query oilMachines($locale: SiteLocale!){
  oilMachines: allMachines(locale: $locale, filter: {slug: {neq: null}, product: {eq: "109839746"}}) {
    slug
  }
}
`;
...
query wineMachines($locale: SiteLocale!){
  wineMachines: allMachines(locale: $locale, filter: {slug: {neq: null}, product: {eq: "109839783"}}) {
    slug
  }
}
`;
...
query beerMachines($locale: SiteLocale!){
  beerMachines: allMachines(locale: $locale, filter: {slug: {neq: null}, product: {eq: "109839755"}}) {
    slug
  }
}
`;
...
query spiritMachines($locale: SiteLocale!){
  spiritMachines: allMachines(locale: $locale, filter: {slug: {neq: null}, product: {eq: "109839737"}}) {
    slug
  }
}
`;
...
query machine($slug: String!, $locale: SiteLocale!, $product: ItemId){
  machine(filter: {slug: {eq: $slug}, product: {eq: $product}}, locale: $locale) {
    slug
    id
    titleHero
    titleDoc
...
query configurator($locale: SiteLocale!) {
  confPage(locale: $locale) {
    title
    id
    slug
  ...
  allMachineConfs(locale: $locale) {
    ...
    functions {
      ...
      images {
        url
        responsiveImage(sizes: "600px", imgixParams: {auto: [format, compress], fit: crop, ar: "16:9"}) {
          ...imgFrag
        }
      }
    }
    previewImage {
      url
      responsiveImage(sizes: "600px", imgixParams: {auto: [format, compress], fit: max}) {
        ...imgFrag
      }
    }
    configurationImages {
      ... on LinkFunctionRecord {
        id
        model: _modelApiKey
        links {
          id
  ...
  allOptionalConfs(locale: $locale) {
    id
    title
