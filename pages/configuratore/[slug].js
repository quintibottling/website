...
const TIER_LEVELS = { start: 'Livello 1', intermediate: 'Livello 2', pro: 'Livello 3' };
...
export default function ConfiguratoreMachine({
  ...
  const [functionState, setFunctionState] = useState(() => {
    ...
    allFunctions.forEach(fn => {
      // Active if it belongs to current machine
  ...
  const getFunctionMachine = useCallback((functionId) => {
    // Sort machines by tier ascending
  ...
  const determineRequiredMachine = useCallback((testFunctionState) => {
    // Sort machines by tier descending (highest first)
    const sortedMachines = [...allMachines].sort(
      (a, b) => TIER_ORDER[b.slug] - TIER_ORDER[a.slug]
    );

    // Find the highest tier machine that has all its functions active
    for (const machine of sortedMachines) {
  ...
  const toggleFunction = useCallback((functionId) => {
    ...
    if (!fn || fn.fixed) return; // Can't toggle fixed functions
  ...
  const confirmNavigation = useCallback(() => {
    if (pendingNavigation) {
      // Store state in sessionStorage for the new page
  ...
  return (
    <Layout site={site} locale={locale} model="configurator">
      <Head>
        <title>Diamond Oil {currentMachine.title} - Configuratore | Quinti Bottling</title>
        <meta
          name="description"
        ...
        <div className="container--standard">
          ...
          <div className="mb-8 flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            ...
            <div className="rounded-lg border border-pink bg-white px-6 py-4">
              <div className="text-xs uppercase text-black/50">Configurazione Attuale</div>
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2">
              <div className="rounded-xl border border-pink bg-white p-6">
                <h2 className="mb-6 text-lg font-semibold text-black">Moduli</h2>

                {/* Fixed Functions */}
                <div className="mb-6">
                  <h3 className="mb-3 text-xs font-semibold uppercase tracking-wide text-black/50">
                    Funzioni Fisse (sempre presenti)
                  </h3>
                    ...
                    {optionalFunctions.map((fn) => {
                      ...
                      return (
                        <FunctionOption
                          ...
                          onInfo={() => openSidebar(fn, 'function')}
                          badgeText={belongsToCurrentMachine ? 'Incluso' : belongsToMachine?.title}
              ...
              <div className="mt-8 rounded-xl border border-pink bg-white p-6">
                <h2 className="mb-2 text-lg font-semibold text-black">Funzioni Plus</h2>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 rounded-xl border border-pink bg-white p-6">
                <h2 className="mb-6 text-lg font-semibold text-black">Anteprima</h2>
            ...
            <div className="flex flex-col items-center justify-between gap-4 lg:flex-row">
              <div>
                ...
                <p className="text-sm text-white/70">
                  Richiedi un preventivo personalizzato per la tua linea Diamond Oil
                </p>
              </div>
              <button
